...
from flight_bot.config import CURRENT_SLOT_LIMIT, MAX_SEARCH_CONCURRENCY, SLOT_DESIGN_CAPACITY, Settings
...
def test_default_search_cadence_daily_summary_and_slot_policy():
    ...
    assert settings.search_stagger_seconds == 5
    assert settings.browser_block_assets is False
    assert settings.browser_timeout_ms == 60000
    assert settings.slot_active_limit == 10
    assert CURRENT_SLOT_LIMIT == 10
    assert SLOT_DESIGN_CAPACITY == 10
    assert MAX_SEARCH_CONCURRENCY == 2


def test_search_concurrency_cannot_exceed_two():
    assert Settings(_env_file=None, search_concurrency=1).search_concurrency == 1
    assert Settings(_env_file=None, search_concurrency=2).search_concurrency == 2
    with pytest.raises(ValidationError):
        Settings(_env_file=None, search_concurrency=3)
...
def test_production_token_requires_allowlist():
    settings = Settings(_env_file=None, app_env="production", telegram_bot_token="secret", telegram_allowed_chat_ids="")
    with pytest.raises(RuntimeError, match="TELEGRAM_ALLOWED_CHAT_IDS"):
        settings.validate_runtime_security()


def test_production_discord_token_requires_allowlist():
    settings = Settings(_env_file=None, app_env="production", discord_bot_token="secret", discord_allowed_channel_ids="")
